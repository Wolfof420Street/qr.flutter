...
//        paint);
  ...
  void _drawImageOverlay(
    ...
    if (style?.shape == EmbeddedImageShape.circle) {
      // Clip the canvas to a circle if the style requires a circular image.
      // Add this to clip the canvas to a circle before drawing the image.
      canvas.save(); // Save the current state of the canvas.
      canvas.clipRRect(RRect.fromRectXY(dst, size.width / 2,
          size.height / 2)); // Clip the canvas to a circle.
